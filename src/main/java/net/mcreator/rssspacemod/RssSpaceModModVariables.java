package net.mcreator.rssspacemod;
...
public class RssSpaceModModVariables {
	public RssSpaceModModVariables(RssSpaceModModElements elements) {
	...
	public void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.getPlayer().world.isRemote()) {
			...
			if (mapdata != null)
				RssSpaceModMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) event.getPlayer()),
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "rss_space_mod_worldvars";
		...
		public void syncData(IWorld world) {
			...
			if (world instanceof World && !world.isRemote())
				RssSpaceModMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(((World) world)::getDimensionKey),
	...
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "rss_space_mod_mapvars";
		public double PlayerRadiationTimer = 0;
		...
		@Override
		public void read(CompoundNBT nbt) {
			PlayerRadiationTimer = nbt.getDouble("PlayerRadiationTimer");
		}

		@Override
		public CompoundNBT write(CompoundNBT nbt) {
			nbt.putDouble("PlayerRadiationTimer", PlayerRadiationTimer);
			return nbt;
		}
		...
		public void syncData(IWorld world) {
			this.markDirty();
			if (world instanceof World && !world.isRemote())
				RssSpaceModMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new WorldSavedDataSyncMessage(0, this));
		}
	...
	public void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
		if (event.getObject() instanceof PlayerEntity && !(event.getObject() instanceof FakePlayer))
			event.addCapability(new ResourceLocation("rss_space_mod", "player_variables"), new PlayerVariablesProvider());
	}
	...
	private static class PlayerVariablesStorage implements Capability.IStorage<PlayerVariables> {
		...
		public INBT writeNBT(Capability<PlayerVariables> capability, PlayerVariables instance, Direction side) {
			...
			nbt.putDouble("XCord", instance.XCord);
		...
		public void readNBT(Capability<PlayerVariables> capability, PlayerVariables instance, Direction side, INBT inbt) {
			...
			instance.XCord = nbt.getDouble("XCord");
	...
	public static class PlayerVariables {
		...
		public void syncPlayerVariables(Entity entity) {
			if (entity instanceof ServerPlayerEntity)
				RssSpaceModMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) entity),
			...
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer()) {
					PlayerVariables variables = ((PlayerVariables) Minecraft.getInstance().player.getCapability(PLAYER_VARIABLES_CAPABILITY, null)
							.orElse(new PlayerVariables()));
					variables.XCord = message.data.XCord;
