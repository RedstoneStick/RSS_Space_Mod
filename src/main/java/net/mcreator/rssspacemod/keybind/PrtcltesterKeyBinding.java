...
package net.mcreator.rssspacemod.keybind;
...
import net.mcreator.rssspacemod.procedures.PukeGeneratorProcedure;
...
public class PrtcltesterKeyBinding extends RssSpaceModModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public PrtcltesterKeyBinding(RssSpaceModModElements instance) {
		super(instance, 117);
	...
	public void initElements() {
		keys = new KeyBinding("key.rss_space_mod.prtcltester", GLFW.GLFW_KEY_F, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					RssSpaceModMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
